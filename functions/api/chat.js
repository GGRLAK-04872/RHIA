...
function localReply(message,world){
  ...
  if(includesAny(text,["wer bist du","was bist du","was ist rhia","wer ist rhia","erzähl mir wer du bist","erzaehl mir wer du bist","erzähl etwas über dich","erzaehl etwas ueber dich","erzähl mir etwas über dich","erzaehl mir etwas ueber dich","stell dich vor","stelle dich vor","wer genau bist du","was genau bist du","was bedeutet rhia"]))return"Ich bin RHIA – RH Intelligent Assistant. Deine loyale digitale Partnerin und das Assistenzsystem von RH Produktion. Ich unterstütze Mike im Alltag, bei RH Produktion, Shadow Grown und bei meiner eigenen Weiterentwicklung.";
  if(includesAny(text,["was kannst du","wobei kannst du helfen","wobei hilfst du","welche funktionen hast du","was sind deine funktionen","was kannst du alles","wie kannst du mich unterstützen","wie kannst du mich unterstuetzen"]))return"Ich kann Aufgaben, Notizen, Ideen, bestätigte Erinnerungen, Welten, Farben, Fokusmodus, Spracheingabe und Gespräche verwalten. Bekannte Befehle beantworte ich lokal; nur freie Wissens- oder Kreativfragen leite ich nach deiner Kostenbestätigung an die KI weiter.";
  if(includesAny(text,["welche welt ist aktiv","in welcher welt bist du","welche welt nutzt du","wo befinden wir uns","welcher bereich ist aktiv"]))return`Aktiv ist gerade die Welt „${clean(world,80)||"Allgemein"}“.`;
  if(includesAny(text,["wie spät ist es","wie spaet ist es","wie viel uhr ist es","sag mir die uhrzeit","aktuelle uhrzeit"]))return`Es ist ${new Intl.DateTimeFormat("de-DE",{timeZone:"Europe/Berlin",hour:"2-digit",minute:"2-digit"}).format(new Date())} Uhr.`;
  if(includesAny(text,["welches datum haben wir","welcher tag ist heute","was ist heute für ein tag","was ist heute fuer ein tag","sag mir das datum","aktuelles datum"]))return`Heute ist ${new Intl.DateTimeFormat("de-DE",{timeZone:"Europe/Berlin",weekday:"long",day:"2-digit",month:"long",year:"numeric"}).format(new Date())}.`;
  if(includesAny(text,["hörst du mich","hoerst du mich","kannst du mich hören","kannst du mich hoeren"]))return"Ja, Mike. Ich höre dich und habe deine Sprache erkannt.";
  if(/^(hallo|hi|hey|guten morgen|guten tag|guten abend)( rhia)?$/.test(text))return"Hallo Mike. Ich bin bereit. Womit fangen wir an?";
  if(includesAny(text,["vielen dank","danke rhia","danke dir","besten dank"])||text==="danke")return"Gern, Mike.";
  return null;
}
...
function costWarning(message,history){
  ...
  return`${COST_MARKER}\nDiese Frage benötigt OpenAI-Credits. Geschätzte Kosten für diese Anfrage: ungefähr ${formatCent(estimate.lowCent)} bis ${formatCent(estimate.highCent)}. Die tatsächlichen Kosten hängen vor allem von der Antwortlänge und dem Gesprächskontext ab. Soll ich die Anfrage kostenpflichtig ausführen? Antworte mit Ja oder Nein.`;
}
...
export async function onRequestPost(context){
  ...
  if(pendingQuestion&&isNo(incomingMessage))return json({ok:true,reply:"Abgebrochen. Es wurden keine OpenAI-Credits verwendet.",model:"local-zero-credit",local:true,cancelled:true});
  ...
  if(!approved){const freeReply=localReply(message,body?.world);if(freeReply)return json({ok:true,reply:freeReply,model:"local-zero-credit",local:true});if(pendingQuestion)return json({ok:true,reply:"Bitte antworte mit Ja, um die angekündigte kostenpflichtige Anfrage auszuführen, oder mit Nein, um sie abzubrechen.",model:"local-zero-credit",local:true});return json({ok:true,reply:costWarning(message,history),model:"local-cost-check",local:true,requiresConfirmation:true})}
